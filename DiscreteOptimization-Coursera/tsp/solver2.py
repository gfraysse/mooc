...
def find_random_solution(points):
    nodeCount = len(points)
    all_points = [i for i in range(1, nodeCount)]
    result=[]
    prev_index = 0
    total_distance = 0
    while len(all_points) > 1:
        i = random.randrange(1, len(all_points))
        idx = all_points[i]
        result.append(idx)
        total_distance += length(points[idx], points[prev_index])
        # print(all_points, i)
        all_points.remove(idx)
        prev_index = idx
...
# def random_solve_it(input_data):
def solve_it(input_data):
    """Simple travelling salesman problem between cities."""
    ...
    # all_points = build_distance_matrix_from_input(points)

    best_result = []
    min_distance = 99999
    for i in range(5000):
        result, total_distance = find_random_solution(points)
        if total_distance < min_distance:
            min_distance = total_distance
            best_result = result

    # print(solution)
    output_data = '%.2f' % min_distance + ' ' + str(0) + '\n'
